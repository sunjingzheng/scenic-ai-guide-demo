...
export const useGuideStore = defineStore("guide", () => {
  ...
  const avatarConfig = ref<AvatarConfig>({
    ...
    live2d: {
      enabled: true,
      assetBase: "/live2d",
      modelUrl: "/live2d/Resources/Hiyori_pro/hiyori_pro_t11.model3.json",
      coreUrl: "/live2d/Core/live2dcubismcore.js",
  ...
  const currentOutfitImage = computed(() => {
    const name = avatarConfig.value.outfit;
    const found = outfitList.value.find((o) => o.name === name);
    return found ? `http://localhost:8000${found.imageUrl}` : "";
  });

  const currentOutfitModelUrl = computed(() => {
    const name = avatarConfig.value.outfit;
    const found = outfitList.value.find((o) => o.name === name);
    if (found) {
      return `http://localhost:8000${found.modelUrl}`;
    }
    // fallback to default local model
    return avatarConfig.value.live2d?.modelUrl || "";
  });
  ...
  async function loadBaseData() {
    ...
    if (!routes.value.length)
      routes.value = await api.recommendRoutes(currentInterest.value);
  }
  ...
  ttsPlayer.onPlayingChange = (v: boolean) => {
    speaking.value = v;
  };
  ...
  async function ask(text: string, imageUrls: string[] = []) {
    ...
    try {
      ...
      if (
        avatarConfig.value.voiceEnabled !== false &&
        ttsConfig.value?.enabled &&
        result.answer
      ) {
  ...
  async function loadOutfits() {
    try {
      outfitList.value = await api.getOutfits();
    } catch {
      // 静默失败，保持当前列表
    }
  }

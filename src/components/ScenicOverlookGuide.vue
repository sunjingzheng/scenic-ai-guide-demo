...
const feedbackCategory = ref<
  "service" | "route" | "knowledge" | "bug" | "suggestion"
>("service");
...
const overlookSpots: OverlookSpot[] = [
  ...
  },
  {
    ...
    y: 53,
    scale: 1.62,
    prompt: "讲讲五印坛城的藏式建筑和祈福体验。",
  },
];

const selectedSpot = computed(
  () =>
    overlookSpots.find((spot) => spot.id === selectedId.value) ||
    overlookSpots[0],
);
...
function findSpotData(spot: OverlookSpot) {
  return store.spots.find(
    (item) => item.id === spot.id || item.name === spot.name,
  );
}

function buildNarration(spot: OverlookSpot, data?: Spot) {
  if (!data)
    return `${spot.name}位于灵山胜境导览图上的${spot.subtitle}区域。点击其它点位，我会把视角移动过去继续讲解。`;
...
async function submitFeedback() {
  await store.submitFeedback(
    ...
    feedbackContent.value || `正在游览${selectedSpot.value.name}`,
  );
  ...
  <section ref="stageRef" class="overlook-guide" :style="focusStyle">
    ...
    <div class="guide-narrator">
      <div class="narrator-copy">
        ...
        <small v-if="recognitionState === 'unsupported'"
          >当前浏览器不支持语音识别，可以使用右下角对话输入。</small
        >
      </div>
    </div>
...
.hotspot.active .pin {
  ...
  box-shadow:
    0 0 0 9px rgba(96, 200, 172, 0.26),
    0 0 26px rgba(96, 200, 172, 0.5);
}

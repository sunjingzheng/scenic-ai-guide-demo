...
import {
  registerLive2DHandler,
  unregisterLive2DHandler,
} from "../features/live2dBridge";
...
const scripts = [
  resolveAssetUrl(
    props.config?.coreUrl || import.meta.env.VITE_LIVE2D_CORE_URL,
    "/live2d/Core/live2dcubismcore.js",
  ),
  props.config?.pixiUrl ||
    import.meta.env.VITE_PIXI_URL ||
...
const modelUrl = computed(() => {
  if (props.modelUrl) {
    // props.modelUrl may be relative (from store); resolve against repoBase
    if (
      /^(https?:)?\/\//.test(props.modelUrl) ||
      props.modelUrl.startsWith("/")
    ) {
      return props.modelUrl;
    }
    return `${repoBase.replace(/\/$/, "")}/${props.modelUrl.replace(/^\//, "")}`;
  }
  return resolveAssetUrl(
    props.config?.modelUrl || import.meta.env.VITE_LIVE2D_MODEL_URL,
    "/live2d/Resources/Hiyori_pro/hiyori_pro_t11.model3.json",
  );
});
...
function buildMouthFrames(audioBuffer: AudioBuffer, fps = 72) {
  ...
  const frameCount = Math.max(
    1,
    Math.ceil(audioBuffer.length / samplesPerFrame),
  );
  ...
  for (let frame = 0; frame < frameCount; frame += 1) {
    ...
    normalizedFrames[frame] = clamp(
      (smoothed - noise * 0.62) / (loud - noise * 0.62),
      0,
      1,
    );
  }
...
async function loadScriptWithRetry(src: string, retries = 2): Promise<void> {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      await loadScript(src, 10000 + attempt * 5000);
      return;
    } catch (error) {
      if (attempt === retries) throw error;
      console.warn(`[Live2D] 第 ${attempt} 次加载失败，重试中: ${src}`);
      await new Promise((r) => setTimeout(r, 1000 * attempt));
    }
  }
}

/** 脚本之间有依赖关系(Cubism4 依赖 Core + Pixi)，必须按顺序加载 */
...
function resizeModel() {
  ...
  const scale = Math.min(
    (width * 1.04) / modelWidth,
    (height * 1.24) / modelHeight,
  );
...
function startFallbackMouthMotion() {
  ...
  const tick = () => {
    ...
    setParameter(
      "ParamMouthOpenY",
      Math.max(0.06, Math.min(0.72, 0.12 + syllable * 0.46 + phrase * 0.14)),
    );
...
function startMouthSyncFromBuffer(buffer: ArrayBuffer) {
  return new Promise<void>(async (resolve) => {
    ...
    const tick = () => {
      ...
      const syllable = audible
        ? Math.max(
            0,
            Math.sin(elapsedMs / 58) * 0.75 + Math.sin(elapsedMs / 132) * 0.25,
          )
        : 0;
      const target = audible
        ? clamp(
            0.18 + Math.pow(energy, 0.62) * 0.58 + syllable * 0.2,
            0.12,
            0.94,
          )
...
async function mountLive2D() {
  ...
  await nextTick();
  if (disposed || !canvasRef.value || gen !== mountGeneration) return;
  ...
  try {
    ...
    if (disposed || !canvasRef.value || gen !== mountGeneration) {
    ...
    registerLive2DHandler(instanceId, {
      ...
      setMouthOpen: (value: number) => {
        stopFallbackMouthMotion();
        setParameter("ParamMouthOpenY", Math.max(0, Math.min(1, value)));
      },
      triggerMotion,
    ...
    if (disposed || gen !== mountGeneration) return;
...
watch(modelUrl, async (newUrl, oldUrl) => {
  // only react to genuine URL changes after initial mount, not the initial resolution
  if (!newUrl || newUrl === oldUrl || !oldUrl) return;
  stopMouthSync();
